...
#     print("I am rank #"+str(rank))
    ...
    if os.path.isfile(interval_file) and os.stat(interval_file).st_size > 0:
        ...
        if sample_rank == 0:
            ...
            # Let them know we finihed calculating the coverage
            ...
            while(True):
                ...
                if tag == CALCULATE_COVERAGE:
                    ...
                    print("[SYSTEM] [TIME] [MPI] [SEND/RECV] [RECV] [{}] received STOP calculating intervals SIGNAL FROM RANK 0 [{}]".format(str(rank), datetime.now().time()))
                    break
    ...
    if rank == 0:
        for homework in homeworks:
            print(type(homework), len(homework))
